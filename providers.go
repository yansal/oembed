...
var providerEndpoints = make(map[*regexp.Regexp]endpoint)
	...
	for _, p := range providers {
		for _, e := range p.Endpoints {
			...
			for _, s := range e.Schemes {
				...
				providerEndpoints[re] = endpoint{URL: url, Discovery: e.Discovery}
			}
		}
	}
}
...
const providersJSON = `[
    ...
    },
    {
        ...
        "provider_url": "hhttps:\/\/pod-test.univ-lille.fr\/",
        "endpoints": [
            {
                "schemes": [
                    "https:\/\/pod-test.univ-lille.fr\/video\/*"
                ],
                "url": "https:\/\/pod-test.univ-lille.fr\/oembed",
                "discovery": true
            }
        ]
    },
